...
int main(int argc, char *argv[]) {
    ...
    ERR("Creating Socket\n");
    tcpsock = rump_sys_socket(RUMP_PF_INET, RUMP_SOCK_DGRAM, 0);
    if (tcpsock <= 0) {
        die(errno, "socket");
    }
    ...
    for(;;) {
        ...
        //int i;
        while((res = rump_sys_recvfrom(tcpsock, rbuf, bufsize, 0, (struct sockaddr*) &client_addr, &client_addr_size)) > 0) {
            /*
            if (res <= 0) {
                die(errno, "read");
            }
            */
            ...
            res = rump_sys_sendto(tcpsock, wbuf, sizeof(wbuf), 0, (struct sockaddr*) &client_addr, client_addr_size);
            if (res <= 0) {
                die(errno, "write");
            }
        }
